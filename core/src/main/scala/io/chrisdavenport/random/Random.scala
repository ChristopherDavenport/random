...
object Random {
   ...
   * 
   **/
  def scalaUtilRandomN[F[_]: Sync](n: Int): F[Random[F]] = for {
    ref <- Ref[F].of(0)
   ...
   */
  def scalaUtilRandomSeedInt[F[_]: Sync](seed: Int): F[Random[F]] = Sync[F].delay{
   ...
   */
  def scalaUtilRandomSeedLong[F[_]: Sync](seed: Long): F[Random[F]] = Sync[F].delay{
    val sRandom = new SRandom(seed)
    new ScalaRandom[F](sRandom.pure[F]){}
  }
   ...
   */
  def javaUtilRandom[F[_]: Sync](random: java.util.Random): F[Random[F]] = Sync[F].delay{
  ...
  private abstract class ScalaRandom[F[_]: Sync](f: F[SRandom]) extends Random[F]{
    ...
    def nextIntBounded(n: Int): F[Int] = for {
      r <- f
      ...
      for {
        ...
        _ <- Monad[F].whileM_(_n.get.map(_ >= Integer.MAX_VALUE))(
          for {
            ...
            _ <- if ((bits & 1) == 0) _n.get.flatMap(n => offset.update(_ + (n - nextN))) else Applicative[F].unit
            _ <- _n.set(nextN)
          } yield ()
        )
